...
def register_nowplaying_handlers(app: Application):
    ...
    async def nowplaying_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        caption_lines = [
            ...
            f"<code>{elapsed_str}</code> {bar} <code>{total_str}</code>",
        ]
        if url:
    ...
    async def nowplaying_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if action == "queue":
            ...
            if not queue:
                await query.answer("📋 The queue is empty.", show_alert=True)
                return
            ...
            if queue:
                ...
                await query.answer(f"⏭ Skipped! Now: {nxt['title']}", show_alert=False)
            else:
                update_session(chat_id, is_playing=False, current_song="", current_url="")
                if is_vc_active():
                    await stop_in_vc(chat_id)
                await query.answer("⏭ Skipped! Queue is now empty.", show_alert=False)
            ...
            if not song:
                ...
                return
            await query.answer("🎵 Fetching lyrics...", show_alert=False)
            await query.message.reply_text("Use /lyrics to get lyrics for the current song.")

    app.add_handler(CommandHandler("nowplaying", nowplaying_command))
    app.add_handler(CallbackQueryHandler(nowplaying_callback, pattern=r"^np:"))
